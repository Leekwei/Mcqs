...
function shuffleOptions(q){
  ...
  const pairs = q.opts.map((text,i)=>({text,i}))

  const shuffled = shuffle(pairs)

  q.opts = shuffled.map(o=>o.text)

  q.ans = shuffled.findIndex(o=>o.i === q.ans)
...
function startTimer() {
  ...
  // prevent crash if the element does not exist
  if (!timer) {
    return;
  }
...
function startQuiz(setNum){
  ...
  QUESTIONS = shuffle(SETS[setNum])
    .map(q => shuffleOptions(structuredClone(q)))
...
function handleOptionClick(e){
  ...
  applyAnswerState(idx)

}


function applyAnswerState(choice){
  ...
  exp.innerHTML =
    `<strong>${choice===q.ans?"Correct!":"Incorrect."}</strong> ${q.exp}`
