...
function shuffleQuestionOptions(q) {
  ...
  for (var i = 0; i < q.opts.length; i++) {
    pairs.push({ text: q.opts[i], original: i });
  }
  ...
  for (var i = 0; i < pairs.length; i++) {
    newOpts.push(pairs[i].text);
    if (pairs[i].original === q.ans) newAns = i;
  }
...
function startTimer(){
  ...
  timerInterval = setInterval(function(){
    ...
    if (timeLeft <= 300)
      document.getElementById('timer').style.color = 'var(--warn');
...
function startQuiz(setNum){
  ...
  QUESTIONS = shuffle(SETS[setNum]).map(function(q){
...
function applyAnswerState(chosen){
  ...
  expEl.innerHTML =
    '<strong>' +
    (chosen === q.ans ? 'Correct!' : 'Incorrect.') +
    '</strong> ' + q.exp;
...
function showResults(timedOut){
  ...
  if (timedOut)
    verdict =
      'Time ran out. Review your answers below and work on your speed.';
  else if (pct >= 80)
    verdict =
      'Outstanding - you are well prepared for Gottingen.';
  else if (pct >= 65)
    verdict =
      'Solid performance. Review weak areas and you will be ready.';
  else if (pct >= 50)
    verdict =
      'A decent start. Focus on topics where you missed questions.';
  else
    verdict =
      'Use the explanations below to guide your revision.';
...
function buildTopicBreakdown(){
  ...
  for (var t = 0; t < topics.length; t++){
    ...
    var color =
      frac >= 0.7 ? 'var(--accent2)' :
      frac >= 0.5 ? 'var(--warn)' :
      'var(--danger)';

    html +=
      '<div class="tb-row">' +
      '<div class="tb-name">' + topics[t] + '</div>' +
      '<div class="tb-bar-outer">' +
      '<div class="tb-bar-inner" style="width:' +
      (frac * 100) +
      '%;background:' + color +
      '"></div></div>' +
      '<div class="tb-frac">' +
...
function buildReview(){
  ...
  var html = '<h3>Full Question Review</h3>';
  ...
  for (var i = 0; i < QUESTIONS.length; i++){
    ...
    var sc =
      sk ? 'review-skip' :
      (ok ? 'review-correct' : 'review-wrong');

    var sl =
      sk ? 'Skipped' :
      (ok ? 'Correct' : 'Wrong');

    html += '<div class="review-item ' + sc + '">';

    html +=
      '<div class="review-header">' +
      '<span class="review-qnum">' +
      'Q' + (i+1) + ' • ' + q.topic +
      '</span>' +
    ...
    for (var j = 0; j < q.opts.length; j++){
      ...
      html +=
        '<div class="rev-opt ' + oc + '">' +
        '<span class="rev-letter">' +
        letters[j] +
        '</span>' +
        q.opts[j] +
        '</div>';
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  document.getElementById('btn-set1')
    .addEventListener('click',function(){startQuiz(1);});

  document.getElementById('btn-set2')
    .addEventListener('click',function(){startQuiz(2);});

  document.getElementById('btn-set3')
    .addEventListener('click',function(){startQuiz(3);});
  ...
  document.getElementById('btn-next')
    .addEventListener('click',nextOrFinish);
